...
if(isset($_GET['id'])){
$pageid = $_GET['id'];
$DBObject->setFSPage($pageid);

	if($DBObject->checkFSPageExistance()==0){
	$_SESSION['message'] = "Site Error";
	header("Location:manage_categories.php");
	}else{
		$resultset = $DBObject->editFSPageContent();
			if($resultset == 0){
			$_SESSION['message'] = "Site Error";
			header("Location:manage_categories.php");
			}
...
if(isset($_POST['Submit']) && isset($_POST['IsSubmit']) && $_POST['IsSubmit']=="1"){
	...
	$dbresult = $DBObject->EditFSPage($pageid,$pagetitle,$pagecontent,$pagemetakeys,$pagemetadesc);
	$_SESSION['message'] = 	'<p class="redtext">Category Page Content Edited</p>';
	header("Location:edit_freestuff_content.php?id=".$catid);
