...
$DBObject->setPage($pageid);
	...
	if($DBObject->checkSubPageExistance()==0){
	$error = TRUE;
	// do some work here //
	}else{
		$resultset = $DBObject->editPageContent();
			if($resultset == 0){
			$error = TRUE; // do something here
			}
	}
}
...
if(isset($_POST['Submit']) && isset($_POST['IsSubmit']) && $_POST['IsSubmit']=="1"){
	...
	if($resultobj->pid == $parentid){
	$dbresult = $DBObject->EditSubPage($pageid,$pagename,$pagetitle,$pagecontent,$pagemetakeys,$pagemetadesc,$parentid);
	}else{
	$dbresult = $DBObject->EditSubPage($pageid,$pagename,$pagetitle,$pagecontent,$pagemetakeys,$pagemetadesc,$parentid);
	}
...
<table width="100%"  border="0" cellpadding="1" cellspacing="1" bgcolor="#6666CC">
  <tr>
    <td class="heading">&nbsp;CMS &gt; EDIT SUBPAGE </td>
  </tr>
  <tr>
    <td valign="top"><form name="form1" method="post" action="" onsubmit="return validate_form_sub(this);">
      <table width="100%"  border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
        ...
        <tr valign="top">
          ...
          <td width="46%"><input name="pagename" type="text" size="60" value="<?php echo $resultset->name;?>"></td>
        ...
        <tr>
		  ...
		  <?php
			...
			$oFCKeditor->Value = $resultset->data;
			$oFCKeditor->Create() ;
			?>
			</div></td>
          </tr>
        ...
        <tr>
          ...
          <td colspan="3"><input name="pagetitle" type="text" size="60" value="<?php echo $resultset->title;?>"></td>
        </tr>
        <tr>
          <td class="normaltext">&nbsp;Page Meta Keywords </td>
          <td colspan="3"><textarea name="pagemetakeys" cols="46" rows="5"><?php echo $resultset->metakey;?></textarea></td>
        </tr>
        <tr>
          <td class="normaltext">&nbsp;Page Meta Description </td>
          <td colspan="3"><textarea name="pagemetadesc" cols="46" rows="5"><?php echo $resultset->metadesc;?></textarea></td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          <td colspan="3">&nbsp;</td>
