...
if(isset($_GET['id'])){
$pageid = $_GET['id'];
$DBObject->setCatPage($pageid);

	if($DBObject->checkCatPageExistance()==0){
	$_SESSION['message'] = "Site Error";
	header("Location:manage_categories.php");
	}else{
		$resultset = $DBObject->editCatPageContent();
			if($resultset == 0){
			$_SESSION['message'] = "Site Error";
			header("Location:manage_categories.php");
			}
...
if(isset($_POST['Submit']) && isset($_POST['IsSubmit']) && $_POST['IsSubmit']=="1"){
	...
	$dbresult = $DBObject->EditCatPage($pageid,$pagetitle,$pagecontent,$pagemetakeys,$pagemetadesc);
	$_SESSION['message'] = 	'<p class="redtext">Category Page Content Edited</p>';
	header("Location:edit_category_content.php?id=".$pageid);
        ...
        <tr>
          ...
          <td width="80%">            
              <div align="right">
                ...
                <input type="button" name="Cancel" value="Cancel" onclick="window.location.href='manage_categories.php';">
              </div></td>
        </tr>
        <tr>
