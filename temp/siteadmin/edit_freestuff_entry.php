...
$DBObject->setFSEntry($id);
	...
	header("Location:manage_freestuff.php");
		...
		$resultset = $DBObject->editFSEntryContent();
			if($resultset == 0){
			$_SESSION['message'] = "Site Error";
			header("Location:manage_freestuff.php");
			}
...
if(isset($_POST['Submit']) && isset($_POST['IsSubmit']) && $_POST['IsSubmit']=="1"){
	...
	$dbresult_check = $DBObject->CheckFSEntry($website,$category,$id);
	if($dbresult_check==0){

	$dbresult = $DBObject->EditFSEntry($category,$name,$website,$description);

	if($dbresult==1){
	$_SESSION['message'] = "Entry Edited";
	header("Location:manage_freestuff_links.php?pid=".$category);
	}else{
...
<table width="100%"  border="1" cellpadding="4" cellspacing="4" bgcolor="#6666CC">
  <tr>
    <td class="heading">&nbsp;CMS &gt; EDIT FREESTUFF ENTRY </td>
  </tr>
  <tr>
