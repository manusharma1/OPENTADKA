...
if(isset($_POST['Submit']) && isset($_POST['IsSubmit']) && $_POST['IsSubmit']=="1"){
	...
	$dbresult = $DBObject->EditFS($catname,$catid);
...
// Delete the existing related images //
			...
			$directory_path = SITE_DOCUMENTROOT.SITE_FOLDERNAME."/SiteImages/FS/";
...
// Delete the existing related images //
	...
	$image_error = 0;
	$dbresult = $DBObject->UpdateFSImage($finalfilename,$catid); // Update Database
	...
	$dbresult = $DBObject->EditFS($catname,$catid);
			...
			header("Location:edit_freestuff.php?id=".$catid);
			}else{
...
$DBObject->setFS($catid);
	...
	header("Location:manage_freestuff.php");
		...
		$resultset = $DBObject->editFSContent();
			if($resultset == 0){
			$_SESSION['message'] = "Site Error";
			header("Location:manage_freestuff.php");
			}
...
<table width="100%"  border="1" cellpadding="4" cellspacing="4" bgcolor="#6666CC">
  <tr>
    <td class="heading">&nbsp;CMS &gt; EDIT FREE STUFF </td>
  </tr>
	  <tr>
		...
		<tr>
          <td class="normaltext">&nbsp;Category Image </td>
			...
			for($i=0; $i<count($allowed_file_exts); $i++){
			  ...
			  if(file_exists($file_exists)){
			  $flag_cat_img = 1;
			  $category_img_image_file_full_path_name = SITE_HOSTNAME."/".SITE_FOLDERNAME."/SiteImages/FS/".$category_img_image_file_name.".".$allowed_file_exts[$i];
			  }
			}
			...
			?>
			<img src="<?php echo $category_img_image_file_full_path_name;?>" width="20" height="20" />
			</td>
        </tr>
