...
static function URLCreator($string, $method = 'get', $ajaxmethod = 'get', $extrajsfunctioncall = '', $ajaxhtmlelementid = PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE, $isblock = false){
	...
	case 1 :
	if(($module=='cms' && $action=='getContent') || ($module=='products' && $action=='getProductInfo') ){
	$conditions = array();
	$conditions['=']['id'] = $id;
	$sqlObj = new MainSQL();
	...
	}else if($module=='products' && $action=='getProductInfo'){
	$columns = array('producttitle');
	$sql = $sqlObj->SQLCreator('S', 'product', $columns, $conditions, '', '', '');
	}
	$result = $sqlObj->FireSQL($sql);
	if($resultset = $sqlObj->FetchResult($result)){
	if($module=='cms' && $action=='getContent'){
	$urlwithpagetitle = str_replace(' ', '_', $sqlObj->getCleanData($resultset->title));
	}else if($module=='products' && $action=='getProductInfo'){
	$urlwithpagetitle = str_replace(' ', '_', $sqlObj->getCleanData($resultset->producttitle));
	}
...
static function URLNameCreator($string=''){
	...
	$result = $sqlObj->FireSQL($sql);
	if($resultset = $sqlObj->FetchResult($result)){
	return $sqlObj->getCleanData($resultset->name);
	}
	}
...
static function getContentFromURL($string=''){
	...
	$result = $sqlObj->FireSQL($sql);
	if($resultset = $sqlObj->FetchResult($result)){
	return $sqlObj->getCleanData($resultset->data);
	}
	}
...
static function CheckMembersSession(){
	...
	if(isset($_SESSION['MemberLoGGedIn']) && $_SESSION['MemberLoGGedIn'] == 'true' && isset($_SESSION['id']) && isset($_SESSION['WSName']) && $_SESSION['WSName'] == PROJ_NAME){
	if(isset($_COOKIE) && $_COOKIE['ProjectMember']=='PM'.$_SESSION['username'].'pm'){
	setcookie('ProjectMember', 'PM'.$_SESSION['username'].'pm', time()+PROJ_SESSION_TIME_LIMIT);
	}else{
	//self::DestroyMembersSession();
	//self::URLForwarder(self::URLCreator(''));
	}
	}else{
	//self::DestroyMembersSession();
	//self::URLForwarder(self::URLCreator(''));
...
static function DestroyMembersSession(){

	setcookie('ProjectMember', 'PMpm', time()-3600); // Delete Cookie
	...
	$_SESSION['member_session_delete_confirmed'] == 'true';
...
$CKEditor->config['contentsCss'] = constant(_TEMPLATE_CSS_FOR_HTMLEDITOR_CONTENTS);
}
...
return '<textarea name="'.$name.'" class="'.$name.'" style="width:100%">'.$value.'
        </textarea>';
}
