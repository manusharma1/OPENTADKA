...
foreach($conditions as $keys => $values){
	foreach($values as $key => $value){
		...
		switch($keys){
		    ...
		    $setconditions .= '`'. $key . '` LIKE \''.parent::db_input_values_protection($value).'\'';
			break;

			case 'ANDLIKE=':
			case 'AND LIKE =':

		    $setconditions .= ' AND `'. $key . '` LIKE \''.parent::db_input_values_protection($value).'\'';
			break;

			case 'ORLIKE=':
			case 'OR LIKE =':

		    $setconditions .= ' OR `'. $key . '` LIKE \''.parent::db_input_values_protection($value).'\'';
			break;
